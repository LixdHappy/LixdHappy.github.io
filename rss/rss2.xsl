...
<xsl:stylesheet version="3.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="heroCover">
      <xsl:choose>
        <xsl:when test="/rss/channel/item[1]/enclosure[starts-with(@type, 'image/')]/@url">
          <xsl:value-of select="/rss/channel/item[1]/enclosure[starts-with(@type, 'image/')]/@url"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="/rss/channel/item[1]/enclosure[1]/@url"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <html lang="zh-CN">
      ...
      <body>
        <div class="shell">
            ...
            <div class="hero-main">
              ...
              <p class="hero-subtitle">
                <xsl:choose>
                  ...
                  <xsl:otherwise>这里展示的是博客的 RSS2 订阅源，保持简洁、精致与高可读性，让内容本身自然浮现。</xsl:otherwise>
                </xsl:choose>
              </p>

              <div class="hero-meta">
                <div class="meta-pill"><strong><xsl:value-of select="count(/rss/channel/item)"/></strong><span>篇文章</span></div>
                <div class="meta-pill"><strong><xsl:call-template name="fmt-rfc822"><xsl:with-param name="date" select="$updated"/></xsl:call-template></strong><span>最近更新</span></div>
          ...
          <div class="article-list">
            <xsl:for-each select="/rss/channel/item">
              ...
              <xsl:variable name="cover">
                <xsl:choose>
                  <xsl:when test="enclosure[starts-with(@type, 'image/')]/@url">
                    <xsl:value-of select="enclosure[starts-with(@type, 'image/')]/@url"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="enclosure[1]/@url"/>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:variable>
                ...
                <div class="article-main">
                  <div class="article-topline">
                    <xsl:if test="category[1]">
                      <a class="article-category" href="{$link}" target="_blank" rel="noopener noreferrer">
                        <xsl:value-of select="category[1]"/>
                      </a>
                    </xsl:if>
                    <span><xsl:call-template name="fmt-rfc822"><xsl:with-param name="date" select="pubDate"/></xsl:call-template></span>
                    <xsl:if test="author">
                      <span>·</span>
                  ...
                  <p class="article-summary"><xsl:value-of select="description" disable-output-escaping="yes"/></p>

                  <xsl:if test="category">
                    <div class="article-tags">
                      <xsl:for-each select="category[position() &lt;= 6]">
                        <a class="article-chip" href="{$link}" target="_blank" rel="noopener noreferrer">
                          <xsl:value-of select="."/>
                        </a>
                      </xsl:for-each>
                    </div>
          ...
          <xsl:if test="count(/rss/channel/item) &gt; 5">
  ...
  <xsl:template name="fmt-rfc822">
    <xsl:param name="date"/>
    <xsl:value-of select="substring($date, 13, 4)"/>
    <xsl:text>-</xsl:text>
    <xsl:choose>
      <xsl:when test="substring($date, 9, 3)='Jan'">01</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Feb'">02</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Mar'">03</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Apr'">04</xsl:when>
      <xsl:when test="substring($date, 9, 3)='May'">05</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Jun'">06</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Jul'">07</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Aug'">08</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Sep'">09</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Oct'">10</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Nov'">11</xsl:when>
      <xsl:when test="substring($date, 9, 3)='Dec'">12</xsl:when>
    </xsl:choose>
    <xsl:text>-</xsl:text>
    <xsl:value-of select="format-number(number(normalize-space(substring($date, 6, 2))), '00')"/>
  </xsl:template>
